// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  TOKYO NIGHT - Cursor Smear Shader                                           ║
// ║  Based on folke/tokyonight.nvim - Night variant                              ║
// ║  "Fluid motion through the neon-lit streets"                                 ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
//
...
// 4. Using Tokyo Night colors for visual impact
...
// ═══════════════════════════════════════════════════════════════════════════
// Primary: blue (#7aa2f7)  → Vibrant neon trail
// ═══════════════════════════════════════════════════════════════════════════

// Main trail color - Tokyo Night blue (vibrant neon glow)
const vec3 BLUE_COLOR = vec3(0.478, 0.635, 0.969); // #7aa2f7
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Using fixed blue color for consistent Tokyo Night theme
    float gray = dot(BLUE_COLOR, vec3(0.299, 0.587, 0.114));
    const float saturationBoost = 1.8;
    vec3 enhancedColor = clamp(
        mix(vec3(gray), BLUE_COLOR, saturationBoost),
        0.0, 1.0
    );

    // Blend trail color with background
    vec4 originalColor = fragColor;
    fragColor.rgb = mix(fragColor.rgb, enhancedColor, alpha);
